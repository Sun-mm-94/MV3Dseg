...
from network.MV3Dseg_multi_pred import get_model
...
def parse_config():
    ...
    parser.add_argument('--checkpoint', type=str, default=None, help='load checkpoint')  # 'logs/SemanticKITTI/default/version_36/checkpoints/last.ckpt'
    # debug
...
if __name__ == '__main__':
    ...
    profiler = SimpleProfiler(dirpath=f'{log_folder}/{configs.log_dir}/profiler.txt')
    ...
    if configs.checkpoint is not None:
        ...
        if configs.fine_tune or configs.test or configs.pretrain2d:
            my_model = get_model.load_from_checkpoint(configs.checkpoint, config=configs, strict=(not configs.pretrain2d))
        else:
            # continue last training
            my_model = get_model.load_from_checkpoint(configs.checkpoint)
