...
from network.basic_block_c import SparseBasicBlock
...
class point_encoder(nn.Module):
    ...
    def forward(self, features, data_dict):
        inv = data_dict['scale_{}'.format(self.scale)]['coors_inv']
        output = torch_scatter.scatter_mean(features, inv, dim=0)  # Input: last layer's coord and feature
        identity = self.layer_in(features)
        output = self.PPmodel(output)[inv]
        output = torch.cat([identity, output], dim=1)  # last layer's

        v_feat = torch_scatter.scatter_mean(  # # Input: last layer's coors_inv and current's coors_inv
...
class SPVBlock(nn.Module):
    def __init__(self, in_channels, out_channels, indice_key, scale, last_scale, scale_list, spatial_shape):
    ...
    def forward(self, data_dict):
        ...
        # voxel encoder
        v_fea = self.v_enc(data_dict['sparse_tensor'])  # last layer 体素特征编码
        v_fea_inv = torch_scatter.scatter_mean(v_fea.features, coors_inv, dim=0)  # 本层体素特征 - 仅平均特征
        # point encoder
        p_fea = self.p_enc(
            features=data_dict['sparse_tensor'].features+v_fea.features,  # last layer 体素特征： skip connect
            data_dict=data_dict
        )
        ...
        data_dict['layer_{}'.format(self.layer_id)]['voxel_feat'] = v_fea.features  # /1  /2  /4  /8
...
class get_model(LightningBaseModel):
    ...
    def forward(self, data_dict):
        ...
        enc_feats = []  # enc_feats 仅用作3D点云分割预测
        for i in range(self.num_scales):
            enc_feats.append(self.spv_enc[i](data_dict))

        output = enc_feats[-1]
        for i in range(self.num_scales-1):
            coors_inv = data_dict['scale_{}'.format(self.scale_list[-1-i])]['coors_inv']
            output = torch.cat([enc_feats[-2-i], output[coors_inv]], dim=1)

        output = output[data_dict['scale_2']['coors_inv']][data_dict['scale_1']['coors_inv']]
